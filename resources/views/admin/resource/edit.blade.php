    ...
    <form action="{{ route($name.'.update', $model->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="flex flex-wrap justify-center w-full bg-white p-4">
            ...
            @if($field->type === 'select')
                    <x-labeled-select :name="$field->name" :label="$field->label" :required="$field->required" :class="$inputClass" :extra-attributes="$field->extraAttributes">
            ...
            @elseif($field->type === 'password')
                    <x-labeled-input :type="$field->type" :label="$field->label" :name="$field->name" :required="$field->required" :class="$inputClass" :extra-attributes="$field->extraAttributes"/>
            ...
            @else
                <x-labeled-input :type="$field->type" :label="$field->label" :name="$field->name" :required="$field->required" :value="$field->value ?? $model->{$field->name}" :class="$inputClass" :extra-attributes="$field->extraAttributes"/>
            @endif
            @endforeach
