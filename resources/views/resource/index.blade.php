                    ...
                    beforeSend: function() {
                        this.url = encodeURI(
                            decodeURI(this.url).split('&').map(segment => {
                                if (segment.startsWith('search[value]=') && segment.length >
                                    'search[value]='.length && segment.match(
                                        /search\[value\]=(GM|DGM|DC|TC|FA)\d+/)) {
                                    return segment.replace(/search\[value\]=(GM|DGM|DC|TC|FA)(\d+)/,
                                        'search[value]=$2')
                                }
                                return segment
                            }).join('&')
                            ...
                            item.action = actionIcons({
                                ...
                                @can($name . '-update')
                                    @if (Route::has($name . '.edit'))
                                        'edit': '{{ route($name . '.edit', '@') }}'.replace('@', item
                                            .id),
                                    @endif
                                @endcan
                                @can($name . '-approve')
                                    @if (Route::has($name . '.update'))
                                        'approve': '{{ route($name . '.update', '@') }}'.replace('@',
                                            item
                                            .id),
                                    @endif
                                @endcan
                                @can($name . '-delete')
                                    @if (Route::has($name . '.destroy'))
                                        'delete': '{{ route($name . '.destroy', '@') }}'.replace('@',
                                            item.id),
                                    @endif
                                @endcan
                            });
